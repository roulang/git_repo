...
//---
   ...
   */
   ...
   double d4_high=iBands(NULL,PERIOD_CURRENT,i_mid_pd,dev2,0,PRICE_CLOSE,MODE_UPPER,last_bar_shift);
   double d2_low=iBands(NULL,PERIOD_CURRENT,i_mid_pd,dev,0,PRICE_CLOSE,MODE_LOWER,last_bar_shift);
   double d2_high=iBands(NULL,PERIOD_CURRENT,i_mid_pd,dev,0,PRICE_CLOSE,MODE_UPPER,last_bar_shift);
   double sign=iCustom(NULL,PERIOD_CURRENT,"lang_th3_ind",false,i_band_ratio,0,last_bar_shift);
   ...
   //open buy
   if (sign>1 && (!i_file_ctl || g_com==1) && order_pft>=0 && weekday!=1) {
      if (sign==2) {
         ls_tgt_price=d4_low;
      } else {
         ls_tgt_price=d2_low;
   ...
   //open sell
   if (sign<-1 && (!i_file_ctl || g_com==2) && order_pft>=0 && weekday!=1) {
      if (sign==2) {
         ls_tgt_price=d4_high;
      } else {
         ls_tgt_price=d2_high;
      }
