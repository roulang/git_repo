...
//input
input int   i_band_pd=12;
input int   i_band_deviation=2;
...
//---
   ...
   for(int i=st-skip_first_bars;i>=0;i--) {
      signalBuffer[i]=getBandStatus(PERIOD_CURRENT,i,i_band_pd,i_band_deviation);
      
      /*
      //debug
      datetime t=Time[i];
      datetime t1=StringToTime("2018.02.06 12:00");
      if (t==t1) {
         Print("time=",t);
         Print("shift=",i);
         Print("getBandStatus=",signalBuffer[i]);
      }
      */
   }
